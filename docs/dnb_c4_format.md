...
**DNB C4 格式** 是 scRNA-DataHub 支持的 **17 种格式中的"自定义10X MTX"格式**的一个特殊变体。

MGI DNBelab C4 平台使用 dnbc4tools 生成的输出数据采用 10X 兼容格式，文件结构与标准 10X MTX 完全相同，但 `features.tsv` 文件存在格式差异：
- **标准 10X MTX**: `features.tsv` 有 2-3 列（基因ID、基因名、特征类型）
- **DNB C4**: `features.tsv` 只有 1 列（基因名）

scRNA-DataHub 会自动检测并处理这种差异，无需用户手动指定格式。
...
DNB C4 格式属于 **"自定义10X MTX"** 类别（第17种格式），是标准 10X MTX 格式的特殊变体：

| 格式编号 | 格式名称 | 说明 |
|---------|---------|------|
| 1-4 | 标准 10X MTX/H5 | Cell Ranger 输出 |
| 5-16 | 其他标准格式 | H5AD, Loom, CSV等 |
| **17** | **自定义10X MTX** | **包括 DNB C4 等变体** ✅ |

### DNB C4 格式支持

- ✅ **自动检测** - 通过 `features.tsv` 列数判断（1列=DNB C4）
- ✅ **自动处理** - 专门的读取方法 `_read_dnb_c4_mtx()`
- ✅ **自动补充** - 添加缺失的 `gene_ids` 和 `feature_types` 列
- ✅ **完全兼容** - 输出标准 AnnData 对象，与 10X 数据结构一致

### 使用建议

1. **无需特殊操作** - 与10X数据使用完全相同的命令
2. **自动识别** - 工具自动识别 DNB C4 格式并切换处理方式
3. **透明处理** - 用户无感知的格式转换
4. **标准输出** - 输出与 10X 数据完全一致的 H5AD

**DNB C4 用户：直接使用，无需担心格式差异！** ✅
